...
function setAppFlowChart(flowData){
	...
	appMean_res_rateSize = {width:0,height:0,margin_left:48,margin_top:8,margin_right:8,margin_bottom:24};
	appMean_res_rateSize.width = $("#appMeanResRateChart").width();
	appMean_res_rateSize.height = 118;
	...
	appMean_res_rateSVG_g.append("g")			
				      .attr("class", "grid")
				      .attr("transform", "translate(0," + appMean_res_innerHeight + ")")
				      .call(d3.axisBottom(appMean_res_grid_xScale).tickValues([0,5,10,15,20,25,30])
    ...
    appMean_res_rateSVG_g.append("g")
					  ...
					  .call(d3.axisLeft(appMean_res_yScale).tickValues([0,5, 10, 15, 20, 25])); // Create an axis component with d3.axisBottom

	appMean_res_rateSVG_g.append("g")			
				      .attr("class", "grid")
				      .attr("transform", "translate(0,0)")
				      .call(d3.axisLeft(appMean_res_yScale).tickValues([0,5, 10, 15, 20, 25])
	...
	appParticle_group_object = [
					...
					{from:2,to:3, duration_offset:1},
					{from:3,to:6, duration_offset:2.5},
					{from:6,to:7, duration_offset:1}
				];
	...
	appFlowParticle_set = appParticle_group_set.selectAll("circle")
										      .data(function(d,i){
										      	return appFlowParticleArray[i];
										      })
										      .enter()
										      .append("circle")
										      .attr("cx",0)
										      // .attr("cy",function(d,i){
										      // 	return appFlowChartSize.height/2 - flowStrokeScale(flowData.flow)/2 + i/numAppFlowParticle *flowStrokeScale(flowData.flow)
										      // })
										      .attr("cy",function(d,i){
										      	return appFlowChartSize.height/2 - 48/2 + i/numAppFlowParticle *48;
										      })
										      .attr("r",function(d){
										      	return 2* d.rOffset;
										      })
    ...
    appFlowParticle_set.transition()
    				.duration(function(d,i){
    					return 550*appParticle_group_object[d.group].duration_offset;
	    			...
	    			.delay(function(d){
				     ...
				     .attr("cx",function(d,i){
				     	return (appParticle_group_object[d.group].to-appParticle_group_object[d.group].from)*appFlowGridUnit;
				     })
				     .attr("opacity",1);
	...
	appNodeArray = [
		...
		{node_title_eng:"departure_node",node_title_kor:"출발지노드",index:3,value:flowData.source_node,block:flowData.source_node_block},
		{node_title_eng:"service_node",node_title_kor:"서비스",index:6,value:flowData.service,block:0},
		{node_title_eng:"arrival_node",node_title_kor:"도착지노드",index:7,value:flowData.destination_node,block:0}
	];
	...
	appBlockLineScale = d3.scaleLinear().domain([0,1,blockMax]).range([0,2,8]);
  ...
  appFlowChartSVG.append("defs").append("marker")
        .attr("id", "arrow1")
	...
	appFlowTunnel_g.selectAll(".tunnel_ellipse")
			    ...
			    .attr("cx",function(d,i){
			    	var j = parseInt(i/2);
			    	return appFlowTunnelSize.width/2 + j*((-1)**i) *1/maxNumTunnel*appFlowTunnelSize.width
			    })
...
function updateAppFlowChart(flowData){
    ...
    for(i=appMean_res_current_x;0<=i;i--){
    	...
    	if(appMean_res_xAxis_ticks.length==6){
    		break;
    	}
 ...
 //    	console.log(appMean_res_xAxis_ticks);
	...
	// appFlowParticle_set.interrupt();

	console.log("previous circles: " + appParticle_group_set.selectAll("circle")._groups[0].length);
	
	var newParticles = appParticle_group_set.selectAll("circle")
					     .data(function(d,i){
					   		return appFlowParticleArray[i];
					     });
	...
	console.log("After Remove: " + appParticle_group_set.selectAll("circle")._groups[0].length);
	...
	console.log("After Append: " + appParticle_group_set.selectAll("circle")._groups[0].length);

	appParticle_group_set.selectAll("circle")
					   .attr("opacity",0)
					   .attr("cx",0)
					   .attr("cy",function(d,i){
				      		return appFlowChartSize.height/2 - 48/2 + i/numAppFlowParticle *48;
